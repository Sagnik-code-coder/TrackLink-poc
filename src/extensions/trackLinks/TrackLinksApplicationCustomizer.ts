  ...
  public onInit(): Promise<void> {
    ...
    const handleUrlChange = (): void => {
      ...
      if (this.lastRecordedUrl !== currentUrl && !isUrlAlreadyAdded) {
        ...
        // Dynamically determine the page name
        let pageName = document.title || 'Untitled Page';
    
        // Specific handling for "viewlsts.aspx"
        if (currentUrl.includes('viewlsts.aspx')) {
          const siteContentsHeader = document.querySelector('.ms-core-pageTitle h1');
          if (siteContentsHeader && siteContentsHeader.textContent) {
            pageName = siteContentsHeader.textContent.trim();
          }
        }
